...
You can easily register new (or override existing) beans by configuring them in `grails-app/conf/spring/resources.groovy` which uses the https://grails.apache.org/docs/snapshot/guide/single.html#springdsl[Grails Spring DSL].
...
..
.
include::../../snippets/grails-app/conf/logback.groovy[tag=logger]
----
