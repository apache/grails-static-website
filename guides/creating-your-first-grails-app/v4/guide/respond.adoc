...
<2> `session` is an instance of the Servlet API’s`HttpSession` class, and is available in every controller. We can retrieve and store properties in the session - in this case, we're storing a `String` with the attribute `name` in the session. For more see the https://grails.apache.org/docs/latest/ref/Controllers/session.html[Grails documentation].
...
----
include::../snippets/grails-app/controllers/org/grails/guides/UrlMappings.groovy[lines="6..10"]
----
