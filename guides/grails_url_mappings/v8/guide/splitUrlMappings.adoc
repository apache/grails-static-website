Over time, your applications might end up with many similar URL mappings, or large numbers of mappings that are specific to one part of the app. There are a few ways to organize URL mappings, including the `group` method, as well as using multiple `UrlMappings.groovy` files.
...
Let's create a second `UrlMappings` file for our `ConferenceController` mappings. Create a new file named `ConferenceUrlMappings.groovy`, and edit its contents as shown below:

[source,groovy]
.grails-app/controllers/demo/ConferenceUrlMappings.groovy
----
include::../snippets/grails-app/controllers/demo/ConferenceUrlMappings.groovy[]
----
...
.grails-app/controllers/demo/UrlMappings.groovy
----
include::../snippets/grails-app/controllers/demo/UrlMappings.groovy[]
----
