...
The first mapping is namespaced. The second mapping has two components: a string, defining the URL pattern, and a closure, where options (such as *constraints*) can be specified to change the behavior of the mapping.

TIP: See how to use URL constraints https://grails.apache.org/docs/latest/guide/theWebLayer.html#applyingConstraints[here in the Grails documentation].
...
<2> Note that we're not using any variables in this pattern, so the request must match the URL pattern exactly. Otherwise, it will be handled by the default URL mapping at the top of the file.
