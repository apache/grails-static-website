Open link:https://grails.apache.org/start/[grails.apache.org/start], pick the `web` profile, name the application `tailwind`, set the package to `example`, choose JDK 21, and download the generated zip. Unzip it and `cd` into the `tailwind` directory.
