...
* `@Transactional(propagation = REQUIRES_NEW)` opens a *new* transaction for the listener body. The publisher's transaction has already committed by the time we get here; Spring's transaction synchronization manager is draining its after-commit callbacks before tearing the original transaction context down. The listener cannot piggy-back on a transaction that has already concluded - any GORM work needs a fresh one. `REQUIRES_NEW` opens it and binds it to the calling thread for the duration of the listener.
