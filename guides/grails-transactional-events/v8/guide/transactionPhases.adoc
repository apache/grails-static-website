...
* *`AFTER_COMMIT` listeners run *after* the publisher's transaction commits.* The publisher's GORM session is closed by the time the listener fires. The listener must open its own transaction (`REQUIRES_NEW`) to do any database work.
