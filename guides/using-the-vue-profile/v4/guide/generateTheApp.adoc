...
The `server` project is our Grails application. The profile will generate a Grails app using the `rest-api` profile, which provides support for creating domain classes as restful resources, https://grails.apache.org/docs/snapshot/guide/single.html#cors[CORS enabled by default], as well as JSON and Markup views.
