...
class DownloadTask extends GrailsWebsiteTask {
    ...
    @Internal
    String description = 'Generates download HTML page -> build/temp/download.html'
    ...
    @TaskAction
    void renderDocsPage() {
        def tempDir = new File(outputDir.get().asFile, 'temp').tap { it.mkdir() }
        def outputFile = new File(tempDir, 'download.html').tap { it.createNewFile() }
        outputFile.text =
